...
3. Use its buttons to exercise restore with c#, logout with c#, restore from component.
4. Inspect the configured hosts and triggers, then open
   [SessionWorkflow.cs](../Samples~/DefinitionWorkflow/Runtime/SessionWorkflow.cs). It is the caller
...
- **Restore with C#**: `SessionWorkflow.Restore()`.
- **Logout with C#**: `SessionWorkflow.Logout()`.
- **Restore from component**: `SessionWorkflow.RestoreComponent()`.
